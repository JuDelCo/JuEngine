...
{
template <>
struct hash<JuEngine::Matcher>
{
	std::size_t operator()(const JuEngine::Matcher& matcher) const
	{
...
#define COMPONENT_GET_TYPE_ID(COMPONENT_CLASS) JuEngine::ComponentTypeId::Get<COMPONENT_CLASS>()
#define Matcher_AllOf(...) (JuEngine::Matcher)JuEngine::Matcher::AllOf(std::vector<JuEngine::ComponentId>({ FOR_EACH(COMPONENT_GET_TYPE_ID, __VA_ARGS__) }))
#define Matcher_AnyOf(...) (JuEngine::Matcher)JuEngine::Matcher::AnyOf(std::vector<JuEngine::ComponentId>({ FOR_EACH(COMPONENT_GET_TYPE_ID, __VA_ARGS__) }))
#define Matcher_NoneOf(...) (JuEngine::Matcher)JuEngine::Matcher::NoneOf(std::vector<JuEngine::ComponentId>({ FOR_EACH(COMPONENT_GET_TYPE_ID, __VA_ARGS__) }))
}
